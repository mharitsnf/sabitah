...
func switch_scene(target_scene: Scenes, before_switch_command: Command = null, after_switch_command: Command = null) -> void:
	...
	if before_switch_command:
		await before_switch_command.action()
	...
	await next_scene_data.get_instance().tree_entered

	if !next_scene_data.get_instance().is_node_ready():
		await next_scene_data.get_instance().ready
